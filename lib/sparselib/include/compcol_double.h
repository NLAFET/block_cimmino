...
/*******************************/
       ...
       VECTOR_int t_row() { return rowind_ ;}
       VECTOR_int t_col() { return colptr_ ;}

       double *val_ptr() {double *p = val_.ptr(); return p; }
       int *colptr_ptr() {int *p = colptr_.ptr(); return p; }
       int *rowind_ptr() {int *p = rowind_.ptr(); return p; }

       const VECTOR_double      val(const MV_VecIndex &I) const { return val_(I); }
       const VECTOR_int         row_ind(const MV_VecIndex &I) const { return rowind_(I); }
       const VECTOR_int         col_ptr(const MV_VecIndex &I) const { return colptr_(I);}

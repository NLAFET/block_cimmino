...
{                                                                      
    ...
    inline double squaredSum(){
        double s = 0;
        for(int i=0; i<dim_; i++) s+= p_[i]*p_[i];
        return s;
    };

    double* ptr() { return p_; }
...
#                   endif
    ...
    inline MV_Vector_double operator/(double alpha){
        MV_Vector_double v_(*this);
        for(int i=0; i<v_.size(); i++) v_[i]/=alpha;
        return v_;
    }
